 ...
 * (Same as packages/cli/src/mouse.ts; the relay ships separately from the CLI.)
 *
 * The screen serializer keeps the tracking mode but not the event encoding
 * (e.g. SGR 1006). Without this, a phone that joins mid-session sends mouse
 * reports in the old X10 format, which an app like Claude Code misreads.
 */
